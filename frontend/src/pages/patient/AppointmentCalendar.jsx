    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-1">
          <button
            onClick={irAnterior}
            className="w-9 h-9 flex items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 hover:text-blue-600 transition-colors"
          >
            <ChevronLeft size={18} />
          </button>
          ...
          <button
            onClick={irHoy}
            className="ml-2 px-4 py-1.5 text-sm font-semibold rounded-lg border border-slate-300 text-slate-600 hover:border-blue-400 hover:text-blue-600 hover:bg-blue-50 transition-colors"
          >
            Hoy
          </button>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200">
          <button
            ...
            className={`px-4 py-1.5 text-sm font-semibold rounded-md transition-all ${
              vista === 'dia' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            Día
          </button>
          <button
            ...
            className={`px-4 py-1.5 text-sm font-semibold rounded-md transition-all ${
              vista === 'semana' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            Semana
          </button>
        ...
        <div className="overflow-x-auto">
          ...
          >
            ...
            {diasVisibles.map((dia) => (
              <div
                key={toISODate(dia)}
                className={`flex flex-col items-center justify-center py-3 border-b border-r border-slate-100 last:border-r-0 relative ${
                  esHoy(dia) ? 'bg-blue-50' : 'bg-slate-50'
                }`}
              >
                {esHoy(dia) && (
                  <span className="absolute top-1.5 w-1.5 h-1.5 rounded-full bg-blue-600" />
                )}
                <span className="text-xs text-slate-400 uppercase tracking-wide">{formatDiaCorto(dia)}</span>
                <span className={`text-lg font-bold ${esHoy(dia) ? 'text-blue-600' : 'text-slate-700'}`}>
                  {formatDiaNumero(dia)}
                </span>
              </div>
            ))}
                ...
                {diasVisibles.map((dia) => {
                  ...
                  return (
                    <div
                      ...
                      className={`min-h-[56px] border-r border-b border-slate-100 last:border-r-0 p-1 transition-colors ${
      ...
      {citaSeleccionada && (
        ...
        >
          <div
            className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 relative animate-in fade-in zoom-in duration-150"
